...
- **New chain pattern — slowed+reverb toolkit:** `quest volume` → `quest speed` → `quest pitch` → `quest reverb` → `quest glue` produces a fully bass-boosted, slowed-down, pitch-down, cathedral-drenched version of a song glued back onto its original music video. All five quests are chain-friendly (each produces a single output file that the next chain can reference by name). Reverb is the third independent dimension of the toolkit: `speed` changes tempo, `pitch` changes frequency, `reverb` adds spatial ambience — three orthogonal axes that combine into the classic slowed+reverb aesthetic.
...
- **docs/COMMAND_CATALOG.md** — Added section 9.14 `reverb` with full argument table, behavior description, "What reverb is NOT" explainer (clarifying that reverb is independent of speed and pitch), chain pattern for the full slowed+reverb edit, and examples. Updated the "Available quests" table in section 9 to list all 14 quests (was 13) with their descriptions and output naming patterns. Earlier sections 9.3 through 9.13 (the original 11 new side-quests) remain unchanged.
