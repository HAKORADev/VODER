...
- **`chains comment`** is a fourth prebuilt-chains subcommand (joining `build` / `load` / `analyze`). It rewrites an existing `.chain` file in place to add or update step-level `comment:` text and per-input-slot `comment.input.N:` annotations. This is the only way to attach per-input descriptions to a chain — `chains build` only takes a single per-step comment, so the workflow is: build the chain first, then document it with `chains comment`.
...
- **`parse_oneline_args()`** for `mode == 'chains'` now recognizes `'comment'` as a fourth subcommand keyword (alongside `build` / `load` / `analyze`), stored in `result['params']['chains_subcmd']`. `oneline_chains()` dispatches `subcmd == 'comment'` to `handle_comment()`.
...
- Per-input comments surface in three places: the `chains analyze` Markdown report (under each manual input slot), the interactive CLI option 9 input prompt (as `Input note:` under the `Accepted:` line), and the `.chain` file itself.
...
### Added — `chains analyze` "what if" dimension + multi-chain journey narrative

- The `chains analyze` Markdown report now includes a **"What if fixed"** note after each step error, describing what the step would do if the error were corrected. For forward-reference errors: "if the referenced step(s) were moved before this step: step N 'name' would need to be moved before this step, the automated reference would resolve to that step's output file at runtime." For syntax errors with an invalid mode: "if the mode were changed to a valid oneline mode (e.g. tts, sts, ttm, stt, se, sfx, svs, ss, train, quest), the step would execute that mode's pipeline." For syntax errors with a valid mode but bad arguments: "if the oneline syntax were corrected, the step would execute as a `<mode>` command with the provided arguments." This gives the reader a concrete picture of the corrected journey, not just the error.
- **Implementation**: a new `_what_if_fixed(step_idx, chain_step, all_chain_names, step_errors)` helper in `voder.py` inspects the error categories (reference, syntax) and the step's mode token to produce the what-if text. Returns `None` if there are no errors or the error category doesn't have a what-if description, in which case the block is omitted.
- When analyzing 2+ chains, the report now includes a **"Multi-Chain Journey"** section showing the load order, each prebuilt's step count and manual input count, which prior prebuilt names are available for cross-prebuilt reference at each position, and the linearity rule: "prebuilts execute strictly in order. A prebuilt cannot reference a later prebuilt's output — the file does not exist yet at that point. If you need chain B's output in chain A, load B before A." This helps users plan multi-prebuilt sessions and understand why forward references are rejected.
- **Implementation**: a new `_analyze_multi_chain_narrative(chain_results)` helper in `voder.py` builds the section. It's called from `handle_analyze()` only when `len(chain_results) > 1`, and appears between the per-chain detail sections and the Overall Summary.

### Renamed — `chains analyze` → `chains journey` + RPG-like narration

- The `chains analyze` subcommand has been renamed to `chains journey` and completely rewritten as an RPG-like narrative report. `chains analyze` is kept as a backward-compatible alias — both `python voder.py chains analyze "name"` and `python voder.py chains journey "name"` route to the same `handle_journey()` handler and produce the same report. The report filename changed from `voder_analyze_chain_<name>_<ts>.md` to `voder_journey_<name>_<ts>.md`.
...
- **Waypoints summary table**: replaces the old "Step Summary" — same columns (name, type, manual, auto, input comments, comment) but with the RPG-flavored header.
- **The Path Walked**: replaces the old "Journey" section. Each step is a "Waypoint" with:
  ...
  - **Offerings awaited** (replaces "Manual input slots") — per-slot format, voice-profile-eligible marker, per-input guidance from `comment.input.N`
  - **Alternate dimension** block (when the step has errors): "But the step falters. Errors are found:" → error list with fixes → "In another dimension — where the chain took another path, a valid path — what could have happened if the error were the correct thing?" → per-error-category what-if description. The old `_what_if_fixed` was renamed to `_what_if_dimension` and extended to cover `naming` and `format` error categories in addition to `reference` and `syntax`. The what-if text now uses the per-mode persona: "If the oneline syntax had been correct, the Voice Weaver would have executed as a `tts` command with the provided arguments, and the step would have produced its output for the next waypoint."
- **The Saga: How the Chapters Connect** (replaces "Multi-Chain Journey", when 2+ chains): shows the load order as a numbered list with chapter names, step counts, offering counts, and which prior chapter names are available for cross-chapter reference at each position. The linearity rule is restated in RPG terms: "chapters execute strictly in order. A chapter cannot echo from a later chapter — that chapter's output does not exist yet at this point in the story. If you need chapter B's output in chapter A, tell chapter B's story first."
- **The Ledger of the Journey** (replaces "Overall Summary"): a statistics table with RPG-flavored row names — Chapters (prebuilt chains), Waypoints (steps), Offerings awaited (manual inputs), Echoes from prior steps (automated references), Errors found, Whispers (warnings). Plus an **Artisans summoned** table showing the per-mode persona name and step count for every mode used across all chains. When there are errors, an **All Errors** table (chapter, waypoint, category, message, fix) is included.
...
- **Implementation**: new helpers — `_journey_report()` (top-level orchestrator), `_journey_opening()` (opening narrative), `_journey_one_chain()` (per-chain chapter, replaces `_analyze_one_chain`), `_journey_alternate_dimension()` (per-error alternate dimension block), `_what_if_dimension()` (replaces `_what_if_fixed`, extended with naming/format categories and persona-flavored text), `_journey_saga()` (replaces `_analyze_multi_chain_narrative`), `_journey_statistics()` (the ledger), `_journey_epilogue()` (final verdict), `_mode_persona()` (persona lookup), `_human_readable_timestamp()` (date formatting). The old names `_analyze_one_chain`, `_analyze_multi_chain_narrative`, `_what_if_fixed` are kept as thin aliases pointing to the new functions, so any external code that imports them still works. `handle_analyze` is also kept as an alias for `handle_journey`.
- The `handle_build()` output line changed from "Analyze it with: python voder.py chains analyze" to "Journey it with: python voder.py chains journey".
- `parse_oneline_args()` for `mode == 'chains'` now recognizes `'journey'` as a fifth subcommand keyword (alongside `build`, `load`, `analyze`, `comment`). `oneline_chains()` dispatches both `'analyze'` and `'journey'` to `handle_journey()`.
